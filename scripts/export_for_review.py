...
def export_main_content(questions: Dict[str, Any], output_path: str) -> None:
    ...
    fieldnames = [
        ...
        'Correct_Answer', 'Category', 'Difficulty', 'Is_Image_Question',
        'Explanation_EN', 'Explanation_DE', 'Key_Concept_EN', 'Key_Concept_DE',
        'Mnemonic_EN', 'Mnemonic_DE', 'Review_Status', 'Reviewer_Comments'
    ]
...
def export_wrong_answers(questions: Dict[str, Any], output_path: str) -> None:
    ...
    fieldnames = [
        'ID', 'Language', 'Why_A_Wrong', 'Why_B_Wrong', 'Why_C_Wrong',
            ...
            for lang in ['en', 'de', 'tr', 'uk', 'ar']:
                if lang in why_wrong:
                    ...
                    if isinstance(lang_explanations, str):
                        print(f"Warning: Question {question['id']} has string instead of dict for {lang} why_others_wrong")
                        continue
...
def export_multilingual_content(questions: Dict[str, Any], output_path: str) -> None:
    ...
    fieldnames = [
        'ID', 'Language', 'Explanation', 'Key_Concept', 'Mnemonic',
        'Review_Status', 'Reviewer_Comments'
    ]
...
def export_image_questions(questions: Dict[str, Any], output_path: str) -> None:
    ...
    fieldnames = [
        'ID', 'Question', 'Image_Count', 'Image_Descriptions',
        'Review_Status', 'Reviewer_Comments'
    ]
        ...
        for q_id, question in questions.items():
            if question.get('is_image_question', False):
                ...
                for img in images:
                    ...
                    desc = img.get('description', 'N/A').replace('\n', ' ').replace('\r', ' ')
                    context = img.get('context', 'N/A').replace('\n', ' ').replace('\r', ' ')
                    formatted_desc = f"Path: {path} | Description: {desc} | Context: {context}"
