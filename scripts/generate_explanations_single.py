...
class SingleQuestionExplanationGenerator:
    ...
    def determine_question_type(self, question_data: Dict, images_dir: Path) -> str:
        ...
        multiple_images = all(
            (images_dir / f"q{question_id}_{i}.png").exists() 
            for i in range(1, 5)
        )
...
IMPORTANT: Remove the correct answer letter from why_others_wrong explanations (don't explain why the correct answer is wrong)."""
            ...
            if 'why_others_wrong' in result:
                for lang in result['why_others_wrong']:
                    if isinstance(result['why_others_wrong'][lang], dict) and correct_letter in result['why_others_wrong'][lang]:
...
def process_single_question(question_id: int):
    ...
    try:
        ...
        if question_type == "multiple_images":
            explanations = generator.generate_multiple_image_explanation(question_data, images_dir)
        elif question_type == "single_image":
            explanations = generator.generate_single_image_explanation(question_data, images_dir)
        else:  # text_only
        ...
        output_dataset['metadata']['last_processed'] = time.strftime("%Y-%m-%d %H:%M:%S")
        output_dataset['metadata']['progress'] = f"{len(output_dataset['metadata']['processed_questions'])}/{output_dataset['metadata']['total_questions']}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate multilingual explanations for a single question')
    parser.add_argument('--question-id', type=int, default=1, help='Question ID to process (default: 1)')
