...
class DatasetVerifier:
    ...
    REQUIRED_ANSWER_FIELDS = {"explanation", "why_others_wrong", "key_concept", "mnemonic"}
    ...
    def verify_image_question(self, question: Dict[str, Any]) -> None:
        ...
        if not question.get("images"):
            self.errors.append(f"Question {q_id}: Image question missing 'images' field")
        ...
        for img in images:
            ...
            if not img.get("path"):
                ...
                if not img_path.exists():
                    self.errors.append(f"Question {q_id}: Image file not found: {img_path}")
        ...
        if q_id in self.CRITICAL_IMAGE_QUESTIONS:
            if len(images) == 0:
                self.errors.append(f"CRITICAL: Question {q_id} is known problematic but has no images!")
    ...
    def verify_multilingual_answers(self, question: Dict[str, Any]) -> bool:
        ...
        for lang in self.REQUIRED_LANGUAGES:
            ...
            if not isinstance(lang_answers, dict):
                self.errors.append(f"Question {q_id}: Invalid format for language '{lang}'")
                complete = False
                continue
            ...
            for field in self.REQUIRED_ANSWER_FIELDS:
                if field not in lang_answers:
                    self.errors.append(f"Question {q_id}: Missing '{field}' in language '{lang}'")
                    complete = False
                elif not lang_answers[field]:
                    self.warnings.append(f"Question {q_id}: Empty '{field}' in language '{lang}'")
    ...
    def verify_question_structure(self, question: Dict[str, Any]) -> None:
        ...
        if not isinstance(options, list) or len(options) != 4:
            ...
            if correct and correct not in options:
                self.errors.append(f"Question {q_id}: Correct answer '{correct}' not in options")
    ...
    def verify_dataset(self) -> bool:
        ...
        for q_id in self.CRITICAL_IMAGE_QUESTIONS:
            if q_id not in detected_image_questions:
                self.errors.append(f"CRITICAL: Known image question {q_id} not detected as image question!")
                self.stats["critical_issues"] += 1
...
def main():
    ...
    parser.add_argument("--dataset", type=Path, default=Path("data/questions.json"),
