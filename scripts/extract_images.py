...
class PDFImageExtractor:
    ...
    def get_page_images(self, page_number: int) -> List[Dict]:
        ...
        for img_index, img in enumerate(image_list):
            try:
                ...
                if not image_rects:
                    logger.debug(f"No position info for image {img_index} on page {page_number}")
                    continue
                ...
                if pix.width < self.min_width or pix.height < self.min_height:
                    logger.debug(f"Skipping small image {pix.width}x{pix.height} on page {page_number}")
                    pix = None
                    continue
        ...
        logger.info(f"Page {page_number}: Found {len(extracted_images)} valid images (excluded logos)")
    ...
    def extract_images_for_question(self, question_id: int, page_number: int, expected_count: Optional[int] = None) -> List[str]:
        """Extract images for a specific question with proper naming."""
        images = self.get_page_images(page_number)
        
        if not images:
            logger.warning(f"No images found for question {question_id} on page {page_number}")
            return []
        
        # If expected count is specified, validate
        if expected_count and len(images) != expected_count:
            logger.warning(f"Expected {expected_count} images for Q{question_id}, found {len(images)}")
...
class ImageQuestionHandler:
    ...
    def _is_multiple_image_question(self, question_id: int, question_data: Dict) -> bool:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Extract images from German Integration Exam PDF')
    parser.add_argument('--question-id', type=int, help='Extract images for specific question ID only')
    parser.add_argument('--pdf-path', type=Path, default=Path('data/gesamtfragenkatalog-lebenindeutschland.pdf'), 
                       help='Path to PDF file')
    parser.add_argument('--output-dir', type=Path, default=Path('data/images'),
                       help='Output directory for extracted images')
    parser.add_argument('--dataset-path', type=Path, default=Path('data/direct_extraction.json'),
    ...
    with PDFImageExtractor(args.pdf_path, args.output_dir) as extractor:
        if args.question_id:
            ...
            if not question_data.get('is_image_question', False):
                logger.warning(f"Question {args.question_id} is not marked as image question")
            
            page_num = question_data.get('page_number', 0)
            logger.info(f"Extracting images for question {args.question_id} from page {page_num}")
            
            # Determine expected image count
            is_multiple = question_handler._is_multiple_image_question(args.question_id, question_data)
            expected_count = 4 if is_multiple else None
            
            paths = extractor.extract_images_for_question(args.question_id, page_num, expected_count)
            
            if paths:
                logger.info(f"Successfully extracted {len(paths)} images for question {args.question_id}")
                for path in paths:
                    logger.info(f"  - {path}")
            else:
            ...
            for q_id, page_num, is_multiple in image_questions:
                ...
                expected_count = 4 if is_multiple else None
                paths = extractor.extract_images_for_question(q_id, page_num, expected_count)
                
                if paths:
                    total_extracted += len(paths)
                    logger.info(f"  ✓ Extracted {len(paths)} images")
                else:
                    logger.warning(f"  ❌ No images extracted")
            
            logger.info(f"Extraction complete: {total_extracted} total images extracted")
