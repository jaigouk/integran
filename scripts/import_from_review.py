...
def import_main_content(csv_data: List[Dict[str, str]], dataset: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    for row in csv_data:
        ...
        if question_id not in dataset['questions']:
            changes['validation_errors'].append(f"Question ID {question_id} not found in original dataset")
            continue
        ...
        if not validate_review_status(review_status):
            changes['validation_errors'].append(f"Invalid review status '{review_status}' for question {question_id}")
            continue
        ...
        if row.get('Reviewer_Comments', '').strip():
            ...
            question['review_metadata']['main_content_comments'] = row['Reviewer_Comments'].strip()
        ...
        if review_status == 'APPROVED':
            ...
            if new_en and new_en != original_en:
                ...
                changes['updated_explanations'].append(f"Q{question_id}: Updated EN explanation")
            
            if new_de and new_de != original_de:
                question.setdefault('explanations', {})['de'] = new_de
                changes['updated_explanations'].append(f"Q{question_id}: Updated DE explanation")
            ...
            if new_concept_en and new_concept_en != original_concept_en:
                ...
                changes['updated_concepts'].append(f"Q{question_id}: Updated EN key concept")
            
            if new_concept_de and new_concept_de != original_concept_de:
                question.setdefault('key_concept', {})['de'] = new_concept_de
                changes['updated_concepts'].append(f"Q{question_id}: Updated DE key concept")
            ...
            if new_mnemonic_en and new_mnemonic_en != original_mnemonic_en:
                ...
                changes['updated_mnemonics'].append(f"Q{question_id}: Updated EN mnemonic")
            
            if new_mnemonic_de and new_mnemonic_de != original_mnemonic_de:
                question.setdefault('mnemonic', {})['de'] = new_mnemonic_de
                changes['updated_mnemonics'].append(f"Q{question_id}: Updated DE mnemonic")
    
    return changes


def import_wrong_answers(csv_data: List[Dict[str, str]], dataset: Dict[str, Any]) -> Dict[str, List[str]]:
    """Import wrong answers CSV and update dataset. Returns change log."""
    changes = {
        'updated_wrong_explanations': [],
        'validation_errors': []
    }
    ...
    for row in csv_data:
        ...
        if row.get('Reviewer_Comments', '').strip():
            ...
            question['review_metadata']['wrong_answers_comments'][language] = row['Reviewer_Comments'].strip()
        ...
        for option in ['A', 'B', 'C']:
            ...
            if new_explanation and new_explanation != original_wrong_answers.get(option, ''):
                original_wrong_answers[option] = new_explanation
                changes['updated_wrong_explanations'].append(f"Q{question_id} {language.upper()}: Updated why {option} wrong")
    
    return changes


def import_multilingual_content(csv_data: List[Dict[str, str]], dataset: Dict[str, Any]) -> Dict[str, List[str]]:
    """Import multilingual content CSV and update dataset. Returns change log."""
    changes = {
        'updated_multilingual': [],
        'validation_errors': []
    }
    ...
    for row in csv_data:
        ...
        if row.get('Reviewer_Comments', '').strip():
            ...
            question['review_metadata']['multilingual_comments'][language] = row['Reviewer_Comments'].strip()
        ...
        if updated_fields:
            changes['updated_multilingual'].append(f"Q{question_id} {language.upper()}: Updated {', '.join(updated_fields)}")
    
    return changes


def generate_change_report(all_changes: Dict[str, Dict[str, List[str]]], output_path: str) -> None:
    """Generate a detailed change report."""
    timestamp = datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S UTC")
    
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(f"# Dataset Review Import Report\n\n")
        ...
        for file_name, changes in all_changes.items():
            ...
            for change_type, change_list in changes.items():
                if change_type == 'validation_errors':
                    ...
                    if change_list:
                        f.write(f"### ✅ {change_type.replace('_', ' ').title()} ({len(change_list)})\n")
...
def main():
    parser = argparse.ArgumentParser(description='Import reviewed CSV files back to JSON dataset')
    parser.add_argument(
        '--input-dir',
    ...
    # Print summary
    total_changes = sum(len(changes[key]) for changes in all_changes.values() 
                       for key in changes if key != 'validation_errors')
    total_errors = sum(len(changes.get('validation_errors', [])) for changes in all_changes.values())
