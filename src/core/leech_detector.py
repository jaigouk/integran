...
from src.core.models import FSRSCard, LeechCard, Question, ReviewHistory
...
class LeechDetector:
    ...
    def generate_leech_report(self, user_id: int = 1) -> LeechReport:
        ...
        for analysis in current_leeches:
            ...
            category = analysis.question.category
        ...
        with self.db_manager.get_session() as session:
            ...
            if intervention_type == InterventionType.SUSPEND_TEMPORARILY:
                ...
                if card:
                    ...
                    card.next_review_date = datetime.now(UTC).timestamp()  # Due now
    ...
    def _analyze_leech_card(self, card: FSRSCard, session: Any) -> LeechAnalysis | None:
        ...
        # Determine severity
        severity = self._categorize_by_difficulty(card.lapse_count, success_rate)
