...
class DataBuilder:
    ...
    def _process_images(
        ...
        logger.info(
            f"Found {sum(len(imgs) for imgs in available_images.values())} images across {len(available_images)} pages"
        )
        ...
        question_image_mapping = self._create_comprehensive_image_mapping(
            questions, available_images
        )
    ...
    def _create_comprehensive_image_mapping(
        ...
        for question in questions:
            ...
            if len(bild_options) >= 2:  # This is an image question
                ...
                best_page = self._find_best_image_page_for_question(
                    question, available_images, used_images
                )
        ...
        logger.info(
            f"Successfully mapped {len(used_images)}/{len(all_images)} images to {len(question_image_mapping)} questions"
        )
